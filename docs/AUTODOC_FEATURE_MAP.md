...
| `src/app/layout.tsx` | **Experiencia Unificada** | Mantener una interfaz consistente con la identidad de Mercado Libre. | Estructural |
...
Es la característica diferencial del producto. Utiliza la lógica de "Tendencia detectada" para automatizar la decisión logística del vendedor, sugiriendo envíos a centros de distribución (Full) de forma estratégica.

### KPI Dashboard (Panel de Control)
Transforma datos crudos de la tienda en indicadores accionables. Se enfoca en las 4 dimensiones clave de un vendedor exitoso:
1. Finanzas (Ventas de hoy).
2. Confianza (Reputación).
3. Inventario (Stock).
4. Logística (Mercado Envíos).
...
- **Fricción**: Proporcionando una interfaz familiar (MeLi Look & Feel) que no requiere capacitación.
